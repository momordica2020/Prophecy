...
namespace Prophecy.Astronomy
{
    ...
    public static class Astronomy
    {
        ...
        {
            return Util.Round(DeltaTSeconds(year, month) / 86400.0, 16);
        }
        ...
        {
            return Util.Round(DeltaTSeconds(year, month) / 60.0, 16);
        }
        ...
        public static double Perturbation(double jd)
        {
            ...
            return Util.Round(0.00001 * S / L, 16);
        }
        ...
        public static double VernalEquinox(double year)
        {
            if (year >= 1000 && year <= 3000)
            {
                ...
                return Util.Round(2451623.80984 + 365242.37404 * m + 0.05169 * Math.Pow(m, 2) - 0.00411 * Math.Pow(m, 3) - 0.00057 * Math.Pow(m, 4), 10);
            }
            else
            {
                double m = year / 1000.0;
                return Util.Round(1721139.29189 + 365242.1374 * m + 0.06134 * Math.Pow(m, 2) + 0.00111 * Math.Pow(m, 3) - 0.00071 * Math.Pow(m, 4), 10);
            }
        }
        ...
        public static double TrueNewMoon(double k)
        {
            ...
            return Util.Round(mnm + apt1 + apt2, 10);
        }
        ...
        public static (double K, double JD) MeanNewMoon(double jd)
        {
            ...
            return (k, Util.Round(jdValue, 10));
        }
        ...
        public static double[] MeanSolarTermsJd(double year)
        {
            ...
            for (int i = 0; i < peri.Length; i++)
            {
                mst[i] = Util.Round(ve + peri[i] - peri[0], 10);
            }
        ...
        public static double[] AdjustedSolarTermsJd(double year, int start, int end)
        {
            ...
            for (int i = 0; i < mst.Length; i++)
            {
                ...
                jqs[i] = Util.Round(mst[i] + pert - dtd, 10);
            }

            return jqs.ToList().GetRange(start, end - start).ToArray();
        }
