...
namespace test
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            curJD = Util.NowUTCmsSince19700101(nowDT) / 86400000d - 10957.5 - curTZ / 24d; //J2000起算的儒略日数(当前本地时间)
            JulianDay.setFromJDay(curJD + Util.J2000);
            string Cal_y = JulianDay.Y.ToString();
            string Cal_m = JulianDay.M.ToString();

            curJD = Util.int2(curJD + 0.5);

            // double By = LunarHelper.year2Ayear<string>(this.Cal_y.Text);
            //// C#: 注: 使用上句也可以, 如果在调用泛型方法时, 不指定类型, C# 编译器将自动推断其类型
            double By = Util.year2Ayear(Cal_y);    // 自动推断类型为: string
        ...
        private string RTS1(double jd, double vJ, double vW, double tz)
        {
            ...
            JulianDay.setFromJDay(jd + Util.J2000);
